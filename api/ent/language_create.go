...
func (lc *LanguageCreate) AddQuoteIDs(ids ...int) *LanguageCreate {
	lc.mutation.AddQuoteIDs(ids...)
	return lc
}

// AddQuotes adds the "quotes" edges to the MovieQuote entity.
func (lc *LanguageCreate) AddQuotes(m ...*MovieQuote) *LanguageCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return lc.AddQuoteIDs(ids...)
}
...
func (lc *LanguageCreate) createSpec() (*Language, *sqlgraph.CreateSpec) {
	...
	if nodes := lc.mutation.QuotesIDs(); len(nodes) > 0 {
