...
func (cq *CategoryQuery) QueryMovies() *MovieQuery {
...
func (cq *CategoryQuery) WithMovies(opts ...func(*MovieQuery)) *CategoryQuery {
	query := (&MovieClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withMovies; query != nil {
		if err := cq.loadMovies(ctx, query, nodes,
			func(n *Category) { n.Edges.Movies = []*Movie{} },
			func(n *Category, e *Movie) { n.Edges.Movies = append(n.Edges.Movies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CategoryQuery) loadMovies(ctx context.Context, query *MovieQuery, nodes []*Category, init func(*Category), assign func(*Category, *Movie)) error {
	...
	query.Where(predicate.Movie(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(category.MoviesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.category_movies
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_movies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_movies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
