...
// The MovieQuoteFunc type is an adapter to allow the use of ordinary
// function as MovieQuote mutator.
type MovieQuoteFunc func(context.Context, *ent.MovieQuoteMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f MovieQuoteFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.MovieQuoteMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.MovieQuoteMutation", m)
}
