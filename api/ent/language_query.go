...
func (lq *LanguageQuery) QueryQuotes() *MovieQuoteQuery {
	query := (&MovieQuoteClient{config: lq.config}).Query()
...
func (lq *LanguageQuery) Clone() *LanguageQuery {
	...
	return &LanguageQuery{
		...
		withQuotes: lq.withQuotes.Clone(),
...
func (lq *LanguageQuery) WithQuotes(opts ...func(*MovieQuoteQuery)) *LanguageQuery {
	query := (&MovieQuoteClient{config: lq.config}).Query()
...
func (lq *LanguageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Language, error) {
	...
	if query := lq.withQuotes; query != nil {
		if err := lq.loadQuotes(ctx, query, nodes,
			func(n *Language) { n.Edges.Quotes = []*MovieQuote{} },
			func(n *Language, e *MovieQuote) { n.Edges.Quotes = append(n.Edges.Quotes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (lq *LanguageQuery) loadQuotes(ctx context.Context, query *MovieQuoteQuery, nodes []*Language, init func(*Language), assign func(*Language, *MovieQuote)) error {
	...
	query.Where(predicate.MovieQuote(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(language.QuotesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.language_quotes
		if fk == nil {
			return fmt.Errorf(`foreign-key "language_quotes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "language_quotes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
